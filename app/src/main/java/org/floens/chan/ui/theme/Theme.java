...
public class Theme {
    ...
    public void applyFabColor(FloatingActionButton fab) {
        if (isMaterialYou()) {
            int dynamicAccent = MaterialColors.getColor(fab,
                    com.google.android.material.R.attr.colorPrimaryContainer,
                    accentColor.color);
            fab.setBackgroundTintList(ColorStateList.valueOf(dynamicAccent));
        } else {
    ...
    public boolean isMaterialYou() {
        return name.startsWith("material_you");
    }

    /**
     * Re-resolve all span/post colors from the activity context.
     * This is needed for Material You themes where dynamic colors are only
     * available after DynamicColors applies to the activity.
     */
    public void resolveColorsFromActivity(Activity activity) {
        ...
        quoteColor = ta.getColor(0, quoteColor);
        highlightQuoteColor = ta.getColor(1, highlightQuoteColor);
        linkColor = ta.getColor(2, linkColor);
        spoilerColor = ta.getColor(3, spoilerColor);
        inlineQuoteColor = ta.getColor(4, inlineQuoteColor);
        subjectColor = ta.getColor(5, subjectColor);
        nameColor = ta.getColor(6, nameColor);
        idBackgroundLight = ta.getColor(7, idBackgroundLight);
        idBackgroundDark = ta.getColor(8, idBackgroundDark);
        capcodeColor = ta.getColor(9, capcodeColor);
        detailsColor = ta.getColor(10, detailsColor);
        highlightedColor = ta.getColor(11, highlightedColor);
        savedReplyColor = ta.getColor(12, savedReplyColor);
        selectedColor = ta.getColor(13, selectedColor);
        textPrimary = ta.getColor(14, textPrimary);
        textSecondary = ta.getColor(15, textSecondary);
        textHint = ta.getColor(16, textHint);
        textColorRevealSpoiler = ta.getColor(17, textColorRevealSpoiler);
